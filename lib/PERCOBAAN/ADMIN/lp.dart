...
class _PosisiKeuanganState extends State<PosisiKeuangan>
  ...
  double labaSetelahPajak = 0;
  double labaBulanDepan = 0;
  ...
  void _hitungLabaBulanan(String monthAndYear) {
    labaSetelahPajak = 0;
    labaBulanDepan = 0;
    ...
    for (var i = 0; i < dataTransaksi.length; i++) {
      if (dataTransaksi[i] is Map<String, dynamic>) {
        ...
        double nilaiTransaksi = double.parse(dataTransaksi[i]['nominal'].toString());

        if (monthAndYearTransaction == monthAndYear) {
          labaSetelahPajak += nilaiTransaksi;
        } else if (monthAndYearTransaction == getNextMonthAndYear(monthAndYear)) {
          labaBulanDepan += nilaiTransaksi;
        }
      }
    }
    ...
    double pajak = labaSetelahPajak * pajakPersentase;
    labaSetelahPajak -= pajak;

    // Mengupdate modal bulan depan
    labaBulanDepan += labaSetelahPajak;
  }
  ...
  String nextMonthAndYear = formatBulan.format(nextMonthDate) +
      ' ' +
      DateFormat.y().format(nextMonthDate);
              ...
              Expanded(
                child: TabBarView(
                  children: distinctMonthAndYear.map((monthAndYear) {
                    _hitungLabaBulanan(monthAndYear);
